...
def save_DE_DataCleanup(feature_name, Outlier_Dict, corrDict, correlationId, OrginalDtypes, userId,msg=None,TargetProblemType=None,Feature_name_new = {}):
    Id = str(uuid.uuid4())
    dbconn, dbcollection = open_dbconn('DE_DataCleanup')
    dbcollection.insert_many([{"_id": Id,
                               "CorrelationId": correlationId,
...
def updQdb(correlationId, status, progress, pageInfo, userId,requestId=None, modelName=None, problemType=None, UniId=None,
           retrycount=3,Incremental = False):
    #print("all argments",correlationId, status, progress, pageInfo, userId,requestId)
    dbconn, dbcollection = open_dbconn('SSAI_IngrainRequests')
    ...
    try:
        if not Incremental:
            if not modelName:
                ...
                dbcollection.update_many({'RequestId': data[0].get('RequestId'),"pageInfo": pageInfo}, {'$set': {"CorrelationId": correlationId,
                                                                                                "Status": status,
                                                                                                "Progress": progress,
     ...
     ##            f.write(str(x) + " documents updated. " +str(modelName)+str(progress)+"\n")
        ...
        raise Exception("Error while updating the DB!")
        #print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^",str(e))
    dbconn.close()
...
def insQdb(correlationId, status, progress, pageInfo, userId, requestId, UniId=None,modelName = None):
    Id = str(uuid.uuid4())
    dbconn, dbcollection = open_dbconn('SSAI_IngrainRequests')
    if not progress.isdigit():
        message = progress
...
def logger(logger, cor, level=None, msg=None,uniqueId=None):
    if logger == 'Get':
        ...
        handler = logging.FileHandler(
            logpath + 'Anomalydetection' + '_' + str(d.day) + '_' + str(d.month) + '_' + str(d.year) + '.log')
...
def getFeatureSelectionVariable(correlationId):
    ...
    data_json = list(dbcollection.find({"CorrelationId": correlationId},
                                       {'_id': 0, "FeatureImportance": 1, "Train_Test_Split": 1}))
    # print (data_json)
    ...
    return ({"selectedFeatures": selectedFeatures, "trainTestSplitRatio": trainTestSplitRatio})
...
def insert_EvalMetrics_FI_T(correlationId, modelName, problemType, r2ScoreVal, rmsVal, maeVal, mseVal, timeSpend,
                            ...
                            retrycount=3,anomalyvalues = None):
    try:
        dbconn, dbcollection = open_dbconn("SSAI_RecommendedTrainedModels")
        Id = str(uuid.uuid4())
        r2ScoreVal["error_rate"] = float("%.2f" % round(r2ScoreVal["error_rate"] * 100, 2))
    ...
    try:
        ...
        if request_status:
            if key in request_output.keys():
                ...
                variable=key + " not found in the response."
        ...
        raise Exception("call api function has failed")
        #print(e)
        ...
        if  type(v) != str:
            ...
            if "workitemexternalid" in m:
               ...
               # print("k" , k)
                ...
                if k != 'Environment' and k != "CodeBranch":
                    if 'createdbyproductinstanceuid' in v.columns:
                        v.rename(columns={'createdbyproductinstanceuid':'productinstanceuid'}, inplace=True)
                    #print("m::::",m)
