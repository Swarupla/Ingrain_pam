...
def Read_Data(correlationId,requestId,pageInfo,userId):
    try:
        genericflow = False
        min_df = utils.min_data()
        Incremental = False    
        ...
        #           "IterationTypeUIds"       : [x["Granularity"]]
            ...
            #                        CallCdmAPI(agileAPI,entityDfs,entity,auto_retrain,lastDateDict,config,deltype = None)
                            ...
                            for entity, delType in x["Entities"].items():
                                ...
                                if delType == "Agile/PPM":
                                    ...
                                    raise Exception("please provide correct method. Method should be PPM")
                ...
                utils.updQdb(correlationId, 'P', '15', pageInfo, userId,modelName=None, problemType=None, UniId=None,retrycount=3,Incremental=Incremental,requestId=requestId)
                           ...
                           if result.status_code==200:
                               ...
                               if data.shape[0]==0:
                                   ...
                                   if user_date_column_name not in data.columns:
                                       ...
                                       data["UniqueRowID"] = data.ClientId  + data.EndToEndId + data.IncidentNumber
                                    ...
                                    with concurrent.futures.ThreadPoolExecutor(max_workers=ncores*(5)) as executor:
                                        ...
                                        for future in concurrent.futures.as_completed(future_to_url):
                                            try:
                                                ...
                                                raise Exception(' generated an exception: %s' % (exc))
                                            else:
                                                variable=(' length of results stories is %d ' % ( len(results)))
                ...
                if MultiSourceDfs['File'] != {} and MultiSourceDfs['Entity'] != {}:
                    ...
                    if MultiSourceDfs['File'] != {}:
                        #print("Print Line 1350: ",MultiSourceDfs['File'])
                ...
                if mapping_flag == 'False':                    
                    ...
                    utils.updQdb(correlationId, 'P', '25', pageInfo, userId,modelName=None, problemType=None, UniId=None,retrycount=3,Incremental=Incremental,requestId=requestId)
            ...
            if mapping_flag == 'True':
                utils.updQdb(correlationId, 'P', '30', pageInfo, userId,modelName=None, problemType=None, UniId=None,retrycount=3,Incremental=Incremental,requestId=requestId)
                ...
                utils.logger(logger, correlationId, 'INFO', ('Ingesting_Data' + '\n' + 'Source Type :' + str(message)),str(requestId))
                utils.updQdb(correlationId, 'P', '35', pageInfo, userId,modelName=None, problemType=None, UniId=None,retrycount=3,Incremental=Incremental,requestId=requestId)
    except Exception as e:
        #print(correlationId, 'E', e.args[0], pageInfo, userId)
        #print(Incremental,requestId)
        utils.updQdb(correlationId, 'E', e.args[0], pageInfo, userId,modelName=None, problemType=None, UniId=None,retrycount=3,Incremental=False,requestId=requestId)
        if genericflow:
