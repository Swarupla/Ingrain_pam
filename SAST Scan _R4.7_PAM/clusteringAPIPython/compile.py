...
extensions=('.py')
exclude_directories=set(['Cythonize','main','SSAIutils','offlinePackages_Linux','offlinePackages','$tf','__pycache__','c_venv','nltk_data'])
...
not_needed_files=['compile.py','evaluation_api.py','webapp.py','wsgi.py','data_quality_check_Clustering.py']
...
if platform.system()=='Windows':
    final_pyd_path=destination+'\\clustering'
elif platform.system()=='Linux':
    final_pyd_path=destination+'/clustering'
...
def copytree(src, dst, symlinks=False, ignore=None):
    if not os.path.exists(dst):
        os.makedirs(dst)
    if src in ['/var/www/myWizard.IngrAInClustering.WebAPI.Python','/var/www/myWizard.IngrAInClustering.WebAPI.Python/datapreprocessing','/var/www/myWizard.IngrAInClustering.WebAPI.Python/visualization','/var/www/myWizard.IngrAInClustering.WebAPI.Python/Models']:    
...
########
for i in os.listdir(final_pyd_path):
    if i not in ['datapreprocessing','visualization','Models']:
        s = os.path.join(final_pyd_path, i)
        try:
            os.remove(s)
        except:
            shutil.rmtree(s)
...
if platform.system()=='Windows':
    ...
    root_src_dir=local_path+"\\Cythonize\\clustering\\"
...
def copytree_final(src, dst, symlinks=False, ignore=None):
    ...
    for item in os.listdir(src):
        ...
        d = os.path.join(dst, item)
        if os.path.isdir(s):
            copytree_final(s, d, symlinks, ignore)
        else:
            if not os.path.exists(d) or os.stat(s).st_mtime - os.stat(d).st_mtime > 1:
                shutil.copy2(s, d)
for i in ['/var/www/myWizard.IngrAInClustering.WebAPI.Python/datapreprocessing','/var/www/myWizard.IngrAInClustering.WebAPI.Python/visualization','/var/www/myWizard.IngrAInClustering.WebAPI.Python/Models']:
    shutil.rmtree(i)
