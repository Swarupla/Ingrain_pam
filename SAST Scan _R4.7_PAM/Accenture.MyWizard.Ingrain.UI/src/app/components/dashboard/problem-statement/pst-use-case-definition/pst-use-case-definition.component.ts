import { Component, OnInit, ElementRef, Inject, OnDestroy, EventEmitter, Output, ViewChild, TemplateRef } from '@angular/core';
...
import { PstFeatureMappingComponent } from '../pst-feature-mapping-popup/pst-feature-mapping.component';
...
import { DatasourceChangeComponent } from '../datasource-change-popup/datasource-change.component';
...
@Component({
  selector: 'app-pst-use-case-definition',
  templateUrl: './pst-use-case-definition.component.html',
  styleUrls: ['./pst-use-case-definition.component.scss']
})
export class PstUseCaseDefinitionComponent implements OnInit, OnDestroy {
  ...
  constructor(private route: ActivatedRoute, @Inject(ElementRef) private eleRef: ElementRef,
    private ps: ProblemStatementService, private coreUtilService: CoreUtilsService,
    ...
    private cascadeService: CascadeModelsService, private _modalService: BsModalService, private envService: EnvironmentService, private apiService: ApiService) {
        ...
        tap(submitFlag => {
          if (submitFlag) {
            if (!this.coreUtilService.isNil(this.correlationId)) {
              ...
              this.ps.deleteModelByCorrelationId(this.correlationId).subscribe(
                data => {
  ...
  disableTabs(targetColumn: any) {
    ...
    if (!this.coreUtilService.isNil(nestedParentEle)) {
      ...
      if (!this.ps.getColumnsdata.IsModelDeployed) {
  ...
  setData() {
    ...
    if (this.pageSource === 'true') { // navigating from my models
      ...
      if (this.selectedTemplate && this.modelName) {
        ...
        if (this.ps.isPredefinedTemplate === 'False') {
          this.showSaveAs = true;
          this.showSave = true;
  ...
  getColumns() {
    this.appUtilsService.loadingStarted();
    this.ps.getColumns(this.correlationId, this.isExistingTemplate, this.isNewModel).subscribe(data => {
      ...
      // added for Custom Data
      if (!this.coreUtilService.isNil(data.CustomDataPullType)) {
        this.customQueryViewEnabled = (data.CustomDataPullType.toString().toLowerCase() === CustomDataTypes.Query.toLowerCase()) ? true : false;
        (this.customQueryViewEnabled === true) ? this.getPreviousQuery() : undefined;
        this.customApiViewEnabled = (data.CustomDataPullType.toString().toLowerCase() === CustomDataTypes.API.toLowerCase()) ? true : false;
      }
            // added for Custom Data
      ...
      if (data.ColumnsList != null || data.AvailableColumns != null) {
        ...
        if (data.TargetColumn != null) {
          ...
          this.ps.setTargetColumn(this.targetColumn);
          this.showSelectAll = true;
          //  // this.setClassOnElement('btnSelectAll', 'btn btn-primary');
        ...
        if (data.TargetUniqueIdentifier) {
          ...
          this.ps.setUniqueIdentifier(this.uniqueIdentifierVal);
        ...
        if (data.TimeSeriesColumn) {
          ...
          this.ps.setTimeSeriesColumn(this.timeSeriesColumns);
  ...
  getColumnsForMyModels() {
    this.appUtilsService.loadingStarted();
    this.ps.getColumnsForMyModels(this.correlationId).subscribe(data => {
  ...
  onSelectChange(value, from, to, event?) {
    ...
    if (to === 'targetContainer') {
      ...
      if (textFromColumnlist !== '') {
        ...
        if (this.hasTextDataType(textFromColumnlist) == false) {
          ...
          if (this.isTimeSeriesOn) {
            ...
            if (dataType) {
              ...
              this.ns.error('Target Column should not be of Date datatype');
            }
          } else {
            this.targetAggDisable = true;
            if (!this.coreUtilService.isNil(this.targetColumn)) {
  ...
  hasTextDataType(columnName){
    if(this.coreUtilService.isNil(columnName)){
      return false;
    }else if(this.columnDataTypes[columnName] === 'Text'){
      this.ns.error('Target column should not be of Text Type');
      return true;
    }else{
      return false;
    }
    ...
    const openTemplateAfterClosed = this.dialogService.open(TemplateNameModalComponent, {}).afterClosed.pipe(
      ...
      tap(data => {
        ...
        if (data) {
          ...
          this.ps.isPredefinedTemplate = 'False';
  ...
  save(isNextClicked?) {
    if (!this.readOnly) {
      ...
      this.isDirty = 'No';
      if (this.hasTextDataType(this.latestTargetValue) == false) {
        if (this.readOnly !== 'true') {
          const _this = this;
          ...
          if (this.selectedTemplate === null || this.selectedTemplate === undefined || this.selectedTemplate === '') {
            ...
            // this.coreUtilService.disableTabs((this.currentIndex + 1), 0);
          } else {
          ...
          if (this.targetColumn && this.isValidate(this.problemStatement)) {
            if (this.isTimeSeriesOn) {
              /* if (this.coreUtilService.isNil(this.uniqueIdentifierVal)
              ...
              if (this.validationForDropDownVal && this.targetAggregationVal && this.timeSeriesColumns !== '') {
                ...
                if ((customDaysValue !== null && customDaysValue > 0 && customDaysValue <= 31 && customDaysSteps !== null) || (customDaysValue === null && customDaysSteps === null)) {
                  ...
                  this.ps.invokePython(this.columnData, this.targetColumn, this.inputColumns, correlationIdToSave, this.problemStatement,
              ...
              if (this.inputColumns.size > 0) {
                ...
                this.ps.invokePython(this.columnData, this.targetColumn, this.inputColumns, correlationIdToSave,
  ...
  openFileProgressDialog(filesData, templateName, correlationId) {
    ...
    if (correlationId === '' || correlationId === undefined) {
      if (totalSourceCount > 1) {
        const openDataSourceFileProgressAfterClosed = this.dialogService.open(FileUploadProgressBarComponent,
    ...
    const featureMappingTemplate = this.dialogService.open(PstFeatureMappingComponent,
      {
        ...
        tap(objData => {
          if (objData) {
            ...
            this.ps.isMappedFromTemplate = 'True';
            this.displayUploadandDataSourceBlock = false;
  ...
  openFileUploadWithDragDrop(oldTemplateCorelationId) {
    this._modalService.show(UploadApiComponent, {
      class: 'modal-dialog modal-xl', backdrop: 'static', initialState: {
        ...
        filteredMetricData: this.filteredMetricData,
        deliveryTypeName: this.deliveryTypeName,
        clientUID: this.paramData.clientUID, deliveryConstructUID: this.paramData.deliveryConstructUId, userId: this.userid,
        fromApp: this.fromApp, previousQuery: this.previousQuery
      }
    }).content.uploadedData.subscribe(newFileDetails => {
  ...
  /* setClassOnElement(elementId: string, className: string) {
    const nativeEle = !this.coreUtilService.isNil(this.eleRef) ? this.eleRef.nativeElement : null;
    const parentEle = !this.coreUtilService.isNil(nativeEle) ? nativeEle.parentElement : null;
    if (!this.coreUtilService.isNil(parentEle)) {
      parentEle.querySelector('#' + elementId).className = className;
    }
  } */
  timeSeriesToggle() {
  ...
  openModalDialog() {
    ...
    this._modalService.show(UploadApiComponent, {
      class: 'modal-dialog modal-xl', backdrop: 'static', initialState: {
        useCaseDefinition: true, filteredMetricData: this.filteredMetricData,
        deliveryTypeName: this.deliveryTypeName, Flag: true, clientUID: this.paramData.clientUID, deliveryConstructUID: this.paramData.deliveryConstructUId, userId: this.userid,
        fromApp: this.fromApp, preBuiltModelName: this.modelName, previousQuery: this.previousQuery
      }
  ...
  openModalDialogForSameModelName() {
    this._modalService.show(UploadApiComponent, {
      class: 'modal-dialog modal-xl', backdrop: 'static', initialState: {
        useCaseDefinition: true, filteredMetricData: this.filteredMetricData, deliveryTypeName: this.deliveryTypeName,
        clientUID: this.paramData.clientUID, deliveryConstructUID: this.paramData.deliveryConstructUId, userId: this.userid,
        fromApp: this.fromApp, previousQuery: this.previousQuery
      }
    }).content.uploadedData.subscribe(filesData => {
  ...
  navigateToUseCaseDefinition(correlationId, modelname?) {
    ...
    //   });
    // window.location.reload();
    const url = '/dashboard/problemstatement/usecasedefinition?modelCategory=&displayUploadandDataSourceBlock=false&modelName=' + modelname;
    this.router.navigateByUrl(url);
    window.location.reload();
  }
  ...
  getDynamicEntity(clientUId) {
    this.appUtilsService.loadingStarted();
    this.ps.getDynamicEntity(clientUId, this.paramData.deliveryConstructUId, this.userid).subscribe(
      data => {
        this.appUtilsService.loadingEnded();
  ...
  */
  ...
  redirectToNext() {
    ...
    if (this.coreUtilService.isNil(localStorage.getItem('isModelSelected'))
      ...
      if (this.coreUtilService.isNil(this.uniqueIdentifierVal)) {
        ...
        this.coreUtilService.disableTabs(this.currentIndex, this.breadcrumbIndex, null, true);
  ...
  previous() {
    ...
    //    this.coreUtilService.disableTabs(this.currentIndex, this.breadcrumbIndex, true, null);
    let url = '/dashboard/problemstatement/templates';
    this.router.navigateByUrl(url);
  ...
  closeFMPopup() {
    this.modalRef.hide();
    if (this.router.url.includes('dashboard/problemstatement/templates') === true) {
      window.location.reload();
    }
  }
  ...
  redirectToMyModels() {
    ...
    const url = '/dashboard/problemstatement/templates';
    this.router.navigateByUrl(url);
  }
  ...
  changeProblemStatement() {
   ...
   // custom data related custom methods region starts.
   viewQueryResult(){
    this.showQueryResponseDialog('Query Response');
  }
  viewApiResult(){
    this.showApiResponseDialog('API Response');
  }
  showApiResponseDialog(title) {
    this._modalService.show(ViewResponseComponent, { class: 'modal-dialog modal-xl', backdrop: 'static', initialState: { title: title, apiResponseEnabled : true, correlationId : this.correlationId } }).content.outputData.subscribe(data=>{
      console.log(data);
    });
  }

  showQueryResponseDialog(title) {
    this._modalService.show(ViewResponseComponent, { class: 'modal-dialog modal-xl', backdrop: 'static', initialState: { title: title, queryResponseEnabled : true, correlationId : this.correlationId, previousQuery : this.previousQuery } }).content.outputData.subscribe(query=>{
      console.log(query);
    });
  }

  getPreviousQuery(){
    this.apiService.get('GetCustomSourceDetails', { 'correlationid': this.correlationId, 'CustomSourceType': CustomDataTypes.Query }).subscribe(response => {
      this.previousQuery = response;
    }, error => {
      this.ns.error('something went wrong');
    });
  }
  // custom data related custom methods region ends.
}
