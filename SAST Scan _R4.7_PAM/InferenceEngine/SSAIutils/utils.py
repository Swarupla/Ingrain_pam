...
def gettraincols(corrid):
    ...
    Sample = eval(list(dbcollection.find({"CorrelationId": corrid}))[0]["InputSample"])[0]
...
def getRequestParams(correlationId, requestId):
    dbconn, dbcollection = open_dbconn('IE_RequestQueue')
    if requestId:
...
# def update_config(corrId,metriclist,datelist,dimensionlist,userId):
#     try:
#         dbconn,dbcollection = open_dbconn('IE_Config')
        
#         dbcollection.update_one({"CorrelationId": corrId},
#                               { "$set":{'CorrelationId':corrId,
#                                             'ApplicationId':None,
#                                                 'MetricColumnList':metriclist,
#                                                 'DateColumnList':datelist,
#                                                 'DimensionsList':dimensionlist ,
#                                                 'FeaturesAndCombinations':[],
...
def update_config(corrId,metriclist,datelist,dimensionlist,suggestedDims,filtervalues,bincolumns,numericalcolumns,userId):
    try:
        dbconn,dbcollection = open_dbconn('IE_Config')
        ...
        dbcollection.update_one({"CorrelationId": corrId},
                              { "$set":{'CorrelationId':corrId,
                                            'ApplicationId':None,
                                                ...
                                                'DateColumnList':datelist,
                                                'DimensionsList':dimensionlist ,
                                                'SuggestedDimensionsList':suggestedDims,
        ...
        error_encounterd = "py_mongo_error"
      
    dbconn.close()
    return None

def getBinAndNumericalColumnsFromDB(corrId):
    try:
        dbconn,dbcollection = open_dbconn('IE_Config')
        data_json = dbcollection.find({"CorrelationId" :corrId})
        k=list(data_json)[0]
        bincolumns=k.get('BinColumns')
        numericalcol=k.get('NumericalColumns')
    except errors.PyMongoError as py_mongo_error:
        error_encounterd = "py_mongo_error"
    dbconn.close()
    return  bincolumns,numericalcol




def updateFeaturesAndCombinations(corrId,reqId,metric,datecolumn,metrictype,features,featurecombinations,suggestedfeat,suggestedftcombinations,userId):
    try:
        dbconn,dbcollection = open_dbconn('IE_FeatureCombination')
        
        dbcollection.update_one({"CorrelationId": corrId,"RequestId":reqId,"MetricColumn":metric,"DateColumn":datecolumn},
                              { "$set":{    "MetricColumnType":metrictype,
                                            "Features":features,
                                            "FeatureCombinations":featurecombinations,
                                            "SuggestedFeatures":suggestedfeat,
                                            "SuggestedCombinations":suggestedftcombinations,
                                            'ModifiedBy':userId,
                                            'ModifiedOn':datetime.utcnow()
...
"""
corrId='insurancedata10rows'
metric='Source'
date='Response Date'
"""
# def getFilterValuesFromDB(corrId,metric,date):
#     try:
#         dbconn,dbcollection = open_dbconn('IE_FeatureCombination')
#         data_json = dbcollection.find({"CorrelationId" :corrId,"MetricColumn":metric,"DateColumn":date},projection={'FilterValues':True})
#         k=list(data_json)[0]
#         filterval=k.get('FilterValues')
#         if filterval is None:
#             filtervalues={}
#         else:

#             filtervalues=json.loads(filterval.replace('\u200b',''))
#     except errors.PyMongoError as py_mongo_error:
#     dbconn.close()
#     return filtervalues
...
def getFilterValuesFromDB(corrId,metric,date):
    try:
        ...
        k=list(data_json)[0]
        filterval=k.get('FilterValues')
        EncryptionFlag=getEncryptionFlag(corrId)
        if filterval is None:
            filtervalues={}
        else:
            if EncryptionFlag:
                t = base64.b64decode(filterval) #DE55......................................
                value=(EncryptData.DescryptIt(t)).encode('ascii',"ignore").decode('ascii')
                if value =="":
                    filtervalues={}
                else:
                    filtervalues = eval(value)
            else:  
                filtervalues=json.loads(filterval.replace('\u200b',''))
    except errors.PyMongoError as py_mongo_error:
        error_encounterd = "py_mongo_error"
...
def getEntityFromDB(corrId):
    try:
        dbconn,dbcollection = open_dbconn('IEModels')
        data_json = dbcollection.find({"CorrelationId" :corrId})
        k=list(data_json)[0]
...
def getMetricAndDateFromDB(corrId,requestId):
    try:
        dbconn,dbcollection = open_dbconn('IE_RequestQueue')
        data_json = dbcollection.find({"CorrelationId" :corrId,"RequestId":requestId})
        k=list(data_json)[0]
        paramArgs=ast.literal_eval(k.get('ParamArgs'))
        metric=paramArgs.get("Metric")
        date=paramArgs.get("date")
    except errors.PyMongoError as py_mongo_error:
        error_encounterd = "py_mongo_error"
    dbconn.close()
    return  metric,date
...
# def getDimensionListFromDB(corrId):
#     try:
#         dbconn,dbcollection = open_dbconn('IE_Config')
#         data_json = dbcollection.find({"CorrelationId" :corrId})
#         k=list(data_json)[0]
#         dims_list=k.get('DimensionsList')
#         suggestedlist=k.get('SuggestedDimensionsList')
#         collist=list(set(dims_list+suggestedlist))
#     except errors.PyMongoError as py_mongo_error:
#     dbconn.close()
#     return  collist
...
def get_volumetric_config(corrId,configId,configType):
    try:
        dbconn,dbcollection = open_dbconn('IE_SavedConfig')
        data_json = dbcollection.find({"CorrelationId" :corrId,'InferenceConfigId':configId,'InferenceConfigType':configType})
        k=list(data_json)[0]
        date_column=k.get('DateColumn')
        dimensionlist=k.get('Dimensions')
        tf=k.get('TrendForecast')
        freq=k.get('Frequency')
    except errors.PyMongoError as py_mongo_error:
        error_encounterd = "py_mongo_error"
    dbconn.close()
    return  date_column,dimensionlist,tf,freq


    


def saveConfigInDB(corrId,InferenceConfigId,InferenceConfigType,InferenceConfigName,userId,*args):
    try: 
        dbconn,dbcollection = open_dbconn('IE_SavedConfig')
        values_list=[]
        for i in args:
        ...
        if len(values_list)==4: 
            dbcollection.update_one({"CorrelationId": corrId,'InferenceConfigId':InferenceConfigId,'InferenceConfigType':InferenceConfigType},
                             { "$set":{'CorrelationId':corrId,
                                           'InferenceConfigId':InferenceConfigId,
                                           'InferenceConfigName':InferenceConfigName,
                                           'ApplicationId':None,
                                            'DateColumn':values_list[0],
                                            'TrendForecast':values_list[1],
                                            'Frequency':values_list[2],
                                            'Dimensions':values_list[3],   
                                            'CreatedBy':userId,
                                            'CreatedOn':datetime.utcnow(),
                                            'ModifiedBy':userId,
                                            'ModifiedOn':datetime.utcnow()
            ...
            dbcollection.update_one({"CorrelationId": corrId,'InferenceConfigId':InferenceConfigId,'InferenceConfigType':InferenceConfigType},
                             { "$set":{'CorrelationId':corrId,
                                           ...
                                           'ApplicationId':None,
                                            ...
                                            'MetricColumn':values_list[0] ,
                                            'MetricColumnType':values_list[1],
                                            'DateColumn':values_list[2],
                                            'Features':values_list[3],
                                            'FeatureCombinations':values_list[4],
                                            'CreatedBy':userId,
                                            'CreatedOn':datetime.utcnow(),
                                            'ModifiedBy':userId,
                                            'ModifiedOn':datetime.utcnow()
...
#updateSelectedFeaturesFromUser('#12','If1_test',' Features','dict of featurecombinations')

def getFeaturesAndCombinations(corrId,InferenceConfigId,InferenceConfigType):
    try:
        dbconn,dbcollection = open_dbconn('IE_SavedConfig')
        ...
        data_json = dbcollection.find( 
            { "CorrelationId" :corrId,
              "InferenceConfigId":InferenceConfigId,
              "InferenceConfigType":InferenceConfigType
              })

        features_dict=list(data_json)[0]
        #features_dict=k.get('SelectedFeatureConfigurationDetails')
        
        metric=features_dict.get('MetricColumn')
        #metricColumnType=features_dict.get('MetricColumnType')
        date=features_dict.get('DateColumn')
        selectedFeatures=features_dict.get('Features')
        selectedFeatureCombinations=features_dict.get('FeatureCombinations')
        
        filterval=features_dict.get('FilterValues')
        EncryptionFlag=getEncryptionFlag(corrId)
        if filterval is None:
            filtervalues={}
        else:
    ...
    return metric,date,selectedFeatures,selectedFeatureCombinations,filtervalues
    




def update_inferences(corrId,InferenceConfigId,InferenceConfigType,results,userId):
    try:
        dbconn,dbcollection = open_dbconn('IE_SavedConfigResults')
        dbcollection.update_one({"CorrelationId": corrId,'InferenceConfigId':InferenceConfigId,'InferenceConfigType':InferenceConfigType},
                             { "$set":
                                    {
...
def read_csv(filepath):  #En.................
    ...
    #data_t = pd.read_csv(filepath,encoding='latin-1')
    return data_t
def read_excel(filepath):  #En.................
    ...
    toread.seek(0)  
    data_t = pd.read_excel(toread,engine='openpyxl')
    #data_t = pd.read_excel(filepath,engine='openpyxl')
    return data_t
...
def UsecaseDefinitionDetails(df,correlationId,userId):
    ...
    for col in df.columns:
        ...
        if df[col].dtypes.name in num_type:
            unValues[col] = [np.asscalar(np.array([x])) for x in list(df[col].dropna().unique())]
        else:
            unValues[col] = list(df[col].dropna().unique().astype('str'))
    ...
    try:
        if not Incremental:
            if not modelName:
                ...
                dbcollection.update_many({'RequestId': requestId}, {'$set': {"CorrelationId": correlationId,
                                                                                            "Status": status,
                                                                                            "Progress": progress,
...
def save_data_multi_file(correlationId, pageInfo,requestId, userId, multi_source_dfs, g ,parent, mapping, mapping_flag, ClientUID,
    ...
    if mapping_flag != 'True':
        ...
        if multi_source_dfs['VDSInstaML'] != {}:
            if "vds_InsatML" in multi_source_dfs['VDSInstaML'].keys():
                if isinstance(multi_source_dfs['VDSInstaML']['vds_InsatML'], pd.DataFrame):
                    ...
                    sourceDetails = {"CID": ClientUID, "DUID": DeliveryConstructUId, "InstaID": insta_id,
                                     "Source": "InstaMl_VDS"}
                    if auto_retrain:
                        size = append_data_chunks(data_frame, "IE_IngestData", correlationId, pageInfo, userId,
                        ...
                        columns = list(data_frame.columns)
                        chunks, size = file_split(data_frame)
                        save_data_chunks(chunks, "IE_IngestData", correlationId, pageInfo, userId, columns,Incremental,requestId,
                ...
                if counter == 0 :
                    for key_i in multi_source_dfs['VDSInstaML'].keys():
                        ...
                        if auto_retrain:
                            size = append_data_chunks(data_frame, "IE_IngestData", key_i, pageInfo, userId,
                                                      columns, sourceDetails, colunivals=None, lastDateDict=lastDateDict, previousLastDate = previousLastDate)
                        else:
                            data_frame = UsecaseDefinitionDetails(data_frame,key_i,userId)
                            columns = list(data_frame.columns)
                            chunks, size = file_split(data_frame)
                            save_data_chunks(chunks, "IE_IngestData", key_i, pageInfo, userId, columns,Incremental,requestId,
                                             sourceDetails, colunivals=None, lastDateDict=lastDateDict, previousLastDate = previousLastDate)
                    return "single_file"
                else:
            ...
            if isinstance(multi_source_dfs['Custom']['Custom'], pd.DataFrame):
                ...
                if Incremental:
                    ...
                    size = append_data_chunks(data_frame, "IE_IngestedData", correlationId, pageInfo,
                    ...
                    if key == 'metricDf':
                        if isinstance(multi_source_dfs[key]['Metric'], pd.DataFrame):
                            ...
                            sourceDetails = {"CID": ClientUID, "DUID": DeliveryConstructUId, "Entity": 'MetricCode',
                                             "Source": "VDS"}
                            if auto_retrain:
                                size = append_data_chunks(data_frame, "IE_IngestData", correlationId, pageInfo,
                                                          userId, columns, sourceDetails, colunivals=None,
                                                          lastDateDict=lastDateDict,previousLastDate = previousLastDate)
                            elif Incremental:
                                cols_required = gettraincols(correlationId)
                        ...
                        for key1 in multi_source_dfs[key].keys():
                            if isinstance(multi_source_dfs[key][key1], pd.DataFrame):
                                ...
                                if auto_retrain:
                                    ...
                                    columns = list(data_frame.columns)
                                    chunks, size = file_split(data_frame)
                                    save_data_chunks(chunks, "IE_IngestData", correlationId, pageInfo, userId,
        ...
        if flag_1 or flag_2:
            ...
            sourceDetails = {"CID": None, "DUID": None, "Entity": "Manual", "Source": "FILE"}
            if auto_retrain:
                size = append_data_chunks(final_df, "IE_IngestData", correlationId, pageInfo, userId, columns,
                                          sourceDetails, colunivals=None, lastDateDict=lastDateDict,previousLastDate = previousLastDate)
            elif Incremental:
                cols_required = gettraincols(correlationId)
                unique_id = uniqueIdentifier(correlationId) 
                ...
                chunks, filesize = file_split(final_df)
                save_data_chunks(chunks, "IE_IngestData", correlationId, pageInfo, userId, columns,Incremental,requestId, sourceDetails,
...
class CustomJsonFormatter(jsonlogger.JsonFormatter):
    def add_fields(self, log_record, record, message_dict):
        ...
        if log_record.get('requestId'):
            log_record['RequestId'] = log_record.get('requestId')
        else:
            log_record['RequestId'] = record.uniqueId    
...
def processId():
    pid = os.getpid()
    return(str(pid))
def logger(logger, cor, level=None, msg=None,requestId=None):
    if logger == 'Get':
        d = datetime.now()
        ...
        handler = logging.FileHandler(
            logpath + 'IE_Python_' + str(d.day) + '_' + str(d.month) + '_' + str(d.year) + '.log')
        ...
        if level == 'INFO' and (LogLevel in ['INFO','ALL']):
            ...
            logger.info(msg,extra={"requestId":requestId,"correlationId":cor,"ProcessId":processId()})
            ...
            if msg == 'Trace':
                logger.error('Trace', exc_info=True,extra={"requestId":requestId,"correlationId":cor,"ProcessId":processId()})
            else:
                logger.error(msg)
...
# def logger(logger, cor, level=None, msg=None):
#     if logger == 'Get':
#         d = datetime.now()
#         logger = logging.getLogger(cor)
#         if len(logger.handlers) > 0:
...
#         handler = logging.FileHandler(
#             logpath + str(cor) + '_' + str(d.day) + '_' + str(d.month) + '_' + str(d.year) + '.txt')
#         formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
#         handler.setFormatter(formatter)
#         logger.addHandler(handler)
#         return logger
...
def insert_EvalMetrics_FI_T(correlationId, modelName, problemType, r2ScoreVal, rmsVal, maeVal, mseVal, timeSpend,
                            actual, forecasted, RangeTime, selectedFreq, lastDataRecorded,xlabelname,ylabelname,legend, pageInfo, userId,
                            retrycount=3):
    try:
        dbconn, dbcollection = open_dbconn("SSAI_RecommendedTrainedModels")
        Id = str(uuid.uuid4())
        ...
        if retrycount > 0:
            ...
            insert_EvalMetrics_FI_T(correlationId, modelName, problemType, r2ScoreVal, rmsVal, maeVal, mseVal,
                                    timeSpend, actual, forecasted, RangeTime, selectedFreq,lastDataRecorded,xlabelname,ylabelname,legend, pageInfo, userId,
...
def getDateCol(correlationId):
    ...
    try:
        if data[0]["TemplateUsecaseID"] in ["2dcf1c54-099b-4711-b9a2-f08ad96d71b7"]:
            return '_'
...
def getlastDataPoint(correlationId):
    dbconn, dbcollection = open_dbconn('IE_IngestData')
...
def append_data_chunks(InstaData, collection, correlationId, pageInfo, userId, columns, sourceDetails, colunivals=None,
    ...
    df = data_from_chunks(corid=correlationId, collection="IE_IngestData")
    ...
    dbconn, dbcollection = open_dbconn("IE_IngestData")
    dbcollection.remove({"CorrelationId": correlationId})
    save_data_chunks(chunks, "IE_IngestData", correlationId, pageInfo, userId, columns,Incremental=False,requestId=None,sourceDetails=sourceDetails,
...
def formVDSAuth():
    try:
        ...
        r = requests.post(config['PAD']['tokenAPIUrl'], headers={"username": config['METRIC']['username'],
...
def CustomAuth(AppId):
    ...
    if data.get('Authentication') == 'AzureAD' or data.get('Authentication') == 'Azure':
        ...
        #r = requests.post(url, headers={'Content-Type': "application/x-www-form-urlencoded", "username":username,"password":password})
        if r.status_code == 200:
            token = r.json()['token']
            return token, r.status_code
        else:
    ...
    try:
        ...
        if request_status:
            if key in request_output.keys():
                ...
                else_block = "key not found in the response"
        else:
            return []
    except Exception as e:
        error_encounterd = str(e.args[0])
...
def getVDSAIOPSAzureToken(AppName="VDS(AIOPS)"):
    ...
    if data.get('Authentication') == 'Azure':
        r = requests.post(data.get('TokenGenerationURL'), headers={'Content-Type': "application/x-www-form-urlencoded"},
                          data=creds)
        if r.status_code == 200:
            token = r.json()['access_token']
    return token, r.status_code
...
def getColumnnames(correlationId):
    try:
        dbproconn,dbprocollection = open_dbconn("IE_IngestData")
...
def CallCdmAPI(agileAPI,entityDfs,entity,auto_retrain,lastDateDict,entityArgs,entityAccessToken,correlationId,Incremental=False):
    auth_type = get_auth_type()
...
def MetricFunction(metricArgs,metricDfs,metricsUrl,metricAccessToken):
    auth_type = get_auth_type()
    if auth_type == 'AzureAD':
        metricResults = requests.post(metricsUrl, data=json.dumps(metricArgs),
                                  headers={'Content-Type'  : 'application/json',
                                  'authorization' : 'bearer {}'.format(metricAccessToken)})
    elif auth_type == 'WindowsAuthProvider':
        metricResults = requests.post(metricsUrl, data=json.dumps(metricArgs),
                                  headers={'Content-Type'  : 'application/json'},
                                  auth=HttpNegotiateAuth())
    if metricResults.status_code != 200:
        ...
        if  metricData['Client'] == []:
            ...
            numberOfPages = metricData['TotalPageCount']
            TotalRecordCount = metricData['TotalRecordCount']
            i = 1
            while i < numberOfPages:
...
def getIaUsaCaseData(ClientUID,DeliveryConstructUId,url,cDetails,token):
    auth_type = get_auth_type()
    ...
    if resp.status_code == 200:
        if resp.json()['TotalRecordCount'] != 0:
            workItem = resp.json()['Items']
            entityDataframe = pd.DataFrame(workItem)
            numberOfPages = resp.json()['TotalPageCount']
            TotalRecordCount = resp.json()['TotalRecordCount']
            i = 1
            while i < numberOfPages:
                payload.update({"CurrentPage": i + 1})
            ...
            if len(cDetails["InputParameters"]["ReleaseUID"]) > 0:
                entityDataframe = entityDataframe.iloc[[-1]]
...
def update_ps_ingestdata(DataSetUId,correlationId,pageInfo,userId):
    ...
    dbconn.close()
    if data_json != {}:
        dbconn_ps_ingestdata, dbcollection_ps_ingestdata = open_dbconn("IE_IngestData")       
...
def UsecaseDefinitionDetails_forlarge_file(df,datasetUID,userId):
    ...
    for col in df.columns:
        ...
        unPercent[col] = {"Message" :msg,"Percent":x}
        if df[col].dtypes.name in num_type:
            unValues[col] = [np.asscalar(np.array([x])) for x in list(df[col].dropna().unique())][:10]
        else:
...
def getCascadeInpSample(cascaded_corid):
    ...
    inputSample = eval(data_json[0].get("InputSample"))
...
def cascadePredictionValidations(cascaded_corid, data_frame):
    ...
    if data_frame.shape[0] < 1:
        raise Exception("Number of records in the file is less than 1. Please upload data file with more number of records for Cascade Visualization.")
...
######## cascade visualization related utils functions ########
def exec_post_request(source_url, body_params, customurl_token, page_number):
    body_params.update({'PageNumber':page_number})
    auth_type = get_auth_type()
    if auth_type == 'AzureAD':
        r = requests.post(source_url, headers={'Content-Type': 'application/json','Authorization': 'Bearer {}'.format(customurl_token)}, data=json.dumps(body_params))
    elif auth_type == 'WindowsAuthProvider':
        r =  requests.post(source_url, data=json.dumps(body_params),headers={'Content-Type': 'application/json'}, auth=HttpNegotiateAuth())
