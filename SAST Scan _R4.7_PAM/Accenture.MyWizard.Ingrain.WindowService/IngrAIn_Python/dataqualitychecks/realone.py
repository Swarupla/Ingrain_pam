# -*- coding: utf-8 -*-
"""
Created on Mon Jun 24 15:51:23 2019
...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 25 16:18:23 2019
...
#data formats
DATE_FORMATS = ['%m/%d/%Y %I:%M:%S %p', '%Y/%m/%d %H:%M:%S', '%d/%m/%Y %H:%M', '%m/%d/%Y', '%Y/%m/%d','%m/%d/%Y %H:%M','%d-%m-%Y'] 
        ...
        if data_copy[col].dtype.name in num_type:
            ...
            q_info[col] = quality_string
             ...
             if Quality_Df.Missing[col] > 99:
                q_score.append(0.0)
                quality_string = "Missing > 99 Hense quality score is zero"
                q_info[col] = quality_string
                continue
             if Quality_Df.Missing[col] > 0:
...
def identifyDaType(data_copy,target):
        ...
        for col in names:
            if data_copy[col].dtype == 'object':
                ...
                Avg_word_count = round(sum(word_list)/len(data_copy[col]))
                uniquePercent = len(data_copy[col].unique())/len(data_copy[col])*100        
                #id columns
        ...
        for col in names:
            ...
            if data_copy[col].dtype == 'object':
                ...
                if uniquePercent < 10 and Avg_word_count <= 3 :
                    data_copy[col] =  data_copy[col].astype('category')
        ...
        uniquePercent = len(data_copy[target].unique())/len(data_copy[target])*100   
        if uniquePercent < 10 :
    ...
    for col1, col2 in itertools.combinations(corr_cols, 2):
        ...
        if not x.empty:
            if x[col1].dtype.name in num_type and x[col2].dtype.name in num_type:
                ...
                crossTabData = pd.crosstab(x[col1], x[col2])
    ...
    for col in columnsModified:
        if columns_modifiedDict[col] == 'Text':
            ...
            try:
                ...
                unchangedColumns[col] = columns_modifiedDict[col]
                print('Not able to change data type for the column, please check it')
...
def CalculateStastics(data_copy): 
    ...
    Outlier_Dict = {}
    percent_missing = round(data_copy.isnull().sum() * 100 / len(data_copy),2)
    count_unique = [len(data_copy[col].unique()) for col in data_copy.columns]
    percent_unique = [round(len(data_copy[col].unique())*100/len(data_copy[col]),2) for col in data_copy.columns]
    for col in data_copy.columns:
    ...
    #                Outlier_Dict[col] = outliers
                ...
                percent_outliers.append(len(outliers)*100/len(data_copy[col]))
    ...
    for col in data_copy.columns:
        ...
        if data_copy[col].dtype.name  == 'category':
               # col_vals = [str(x).lower() for x in np.unique(data_copy[col].dropna())]
                ...
                class_Ratio = round(data_copy[col].value_counts()*100/len(data_copy),3).sort_values(ascending = False)
...
def Data_Quality_Checks(data,target,DfModified=None,flag = None,text_cols= None,id_cols=None,OrgnalTypes = None): 
    ...
    if flag == None:
         ...
         data_copy,id_cols,date_cols,text_cols,incorrect_date = identifyDaType(data.copy(),target)
    ...
    Q_Scores,Q_Info = calculate_QScore(data_copy,percent_missing,Outlier_Data_Percent,Skewed,incorrect_date,ImBalanced_col)    
    ...
    Balanced = []
    DateFormat={}
    #Forming recommendation for each data quality check of column
    for col in data_copy.columns:
        dtypelist2 = ["category","float64","int64","Id"]
        dtypelist3 = ["Text","category","datetime64[ns]","Id"]
        ...
        if col == target and data_copy[col].dtype.name  == 'category' :
            if len(np.unique(data_copy[col].dropna())) > 2:
        ...
        if col in id_cols:
            ...
            Data_quality_df.Q_Info[col]  = "Id columns does not add any value hense data quality is zero"
            Dtype = "Id"
        ...
        if Dtype == 'object' :
            ...
            if OrgnalTypes[col] in num_type:
                dtypelist2.remove(Dtype)
                dtypelist2.insert(0,Dtype)
                DtypeDict[col] = dtypelist2
            else:
             
                dtypelist3.remove(Dtype)
                dtypelist3.insert(0,Dtype)
                DtypeDict[col] = dtypelist3
...
def ViewDataQuality(correlationId,pageInfo,userId,flag):                   
    try:
        ...
        utils.logger(logger,correlationId,'INFO',('\n'+'data_quality_check'+'\'n'+"Updation initiated for flag= "+str(flag) +" correlation Id :"+str(correlationId)+ "Data Quality Check"))
        ...
        Quality_df,Outlier_Dict,Skewed_cols,text_cols,target,DtypeDict,ScaleDict,DfModified,\
            corrDict,id_cols,corelated_Series,DateFormat = Data_Quality_Checks(OGData,target,None,"View")
...
def UpdateData(correlationId,pageInfo,userId,flag):                   
    try:
        ...
        df_cleanup,columns_modifiedDict,dtype,scale,scale_modified,OrginalTypes = utils.get_DataCleanUp(correlationId)
        ...
        unchangedColumns,FilterData,text_cols,id_cols =  ChangeDataType(FilterData,columnsModified,columns_modifiedDict)
        
        if len(unchangedColumns.keys()) == columnsModified:
            ...
            # Data Clean up
            Quality_df,Outlier_Dict,Skewed_cols,text_cols,target,DtypeDict,ScaleDict,DfModified,\
            corrDict,id_cols,corelated_Series,DateFormat = Data_Quality_Checks(FilterData.copy(),target,DfModified,flag,text_cols,id_cols,OrginalTypes)
            ...
            if len(DateFormat) > 0:
                ...
                DateFormat = dateFmts
            utils.Update_DataCleanUP_FilteredData(correlationId,ColUniqueValues,typeDict,UserInputColumns,DateFormat)
            ...
            flag = None
            utils.Update_DE_DataCleanup(correlationId,unchangedColumns,flag,feature_name,Outlier_Dict,corrDict,str(TargetProblemType),OrginalTypes)
...
def main(correlationId,pageInfo,userId):                   
    try:
        ...
        utils.logger(logger,correlationId,'INFO',('\n'+'data_quality_check'+'\'n'+"Process initiated for "+str(pageInfo)+" correlation Id :"+str(correlationId)+ "Data Quality Check"))
        ...
        utils.logger(logger,correlationId,'INFO',('\n'+'data_quality_check'+'\'n'+"page info:  "+str(pageInfo)+" Target :"+str(target_variable)+ '\n'+"Input Columns :" +str(UserInputColumns)+ '\n'+ "from PS_BusinessProblem"))        
    ...
    # Data Clean up
        ...
        ScaleDict,data_copy,corrDict,id_cols,corelated_Series,DateFormat = Data_Quality_Checks(data_t[UserInputColumns],target_variable)
        ...
        utils.save_DataCleanUP_FilteredData(target_variable,correlationId,ColUniqueValues,typeDict,UserInputColumns,DateFormat)
        ...
        utils.save_DE_DataCleanup(feature_name,Outlier_Dict,corrDict,correlationId,str(TargetProblemType),OrgDtypes)
