...
def scaler(data,column,scaler):        
    ...
    if scaler == 'StandardScaler':
        ...
        data_t,scaler = BoxCox(data[column])
    elif scaler == 'Reciprocal':
        data_t, scaler = Reciprocal(data[column].values.reshape(-1,1))
...
def remove_outlier(skewedflag, data, columns,wtd):    
    data_t = data
    if skewedflag == 'Yes':
        res = data_t[columns].quantile([0.05,0.95])
        true_inx1=(data_t[columns].values <  res.loc[0.05])
        true_inx2=(data_t[columns].values > res.loc[0.95])
        for true_inx in [true_inx1,true_inx2]:
            if wtd == 'Median':
                data_t[columns][true_inx] = data_t[columns].median()
            elif wtd == 'Mean':
                data_t[columns][true_inx] = data_t[columns].mean()
            elif wtd == 'Mode':
                data_t[columns][true_inx] = data_t[columns].mode()[0]
            elif wtd == 'None':
                pass
            else:
                data_t[columns][true_inx] = wtd
    else:
        if wtd == 'Remove':
