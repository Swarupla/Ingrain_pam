...
def text_process1(t,pos=True,lemmetize=True,stem=True,stop_words=[],language ='english',spacy_vectors= spacy.load('en_core_web_lg')):
    ...
    if language != 'thai': 
        ...
        lemmatized= ""
        doc = spacy_vectors[language]['model'](t)
        for token in doc:
...
def clustering(text,n_cluster,correlationId,language ='english',spacy_vectors=[]):
    #if language=='french':
    #    language='english'
    ...
    if vec=='Glove':
        ...
        if language !='thai':
            X = np.array([spacy_vectors[language]['model'](word).vector for word in allwords]).T
        else:
            X =  np.array([spacy_vectors[language]['model'][word] for word in allwords]).T
...
def clustering_optional(text, correlationId, pageInfo = 'DataPreprocessing',language ='english',spacy_vectors=[]):
    '''
...
#         X_doc, X_sum, X_avg, X_min, X_max, X_minmaxconcat, X_allconcat, X_vecxtfidf = [[] for _ in range(8)]
        ...
        #     tokens = sent.split()
            ...
            if vec=='Glove':
                if language !='thai':
                    sent_vecs = [spacy_vectors[language]['model'](word).vector for word in tokens] # Getting glove vectors for each word in a sentence
                else:
                    sent_vecs =  [spacy_vectors[language]['model'][word] for word in tokens]
            if vec=='Word2Vec':
...
def vectorize(sent,number,vec,aggregation,vectorizer_tfidf,df_tfidf,language,spacy_vectors):
    #print(number)
    vectorizer1.fit_transform([sent])
    tokens = vectorizer1.get_feature_names()
    #print(number)
...
def clustering_optional_multithread(text, correlationId, pageInfo = 'DataPreprocessing',language ='english',spacy_vectors=[]):
    '''
    Creates document level word vectors for Count Vectorizer and Tfidf Vectorizer
    Creates document level aggregation of multidimentional word features for Word2Vec and Glove
    '''
...
#         X_doc, X_sum, X_avg, X_min, X_max, X_minmaxconcat, X_allconcat, X_vecxtfidf = [[] for _ in range(8)]
        ...
        """ if language =='english':
            ...
            spacy_vectors = spacy_vectors_thai """
        if platform.system() == 'Linux':
            X.append(Parallel(n_jobs=3,backend='threading')(
                delayed(vectorize)(text[i],i,vec,aggregation,vectorizer_tfidf,df_tfidf,language,spacy_vectors) for i in range(0,len(text))))
        elif platform.system() == 'Windows':
