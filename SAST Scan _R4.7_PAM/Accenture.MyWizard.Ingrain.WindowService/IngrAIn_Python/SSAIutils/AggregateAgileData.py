...
def Agileaggdata_function(Agile):
    try:
        ...
        Agile['effortestimatedvalue'] = Agile['effortestimatedvalue'].replace('$', 0)
        Agile['effortestimatedvalue'] = Agile['effortestimatedvalue'].replace('$$', 0)
        Agile['effortestimatedvalue'] = Agile['effortestimatedvalue'].apply(pd.to_numeric, errors="ignore")
        Agile['effortestimatedvalue'] = Agile['effortestimatedvalue']/3600
        Agile['effortremainingvalue'] = Agile['effortremainingvalue']/3600
        Agile['effortcompletedvalue'] = Agile['effortcompletedvalue']/3600
    ...
    try:
        ...
        Story = Story.drop_duplicates(subset='UniqueRowID', keep="last")
        Story = Story.drop_duplicates(subset='prdctid_workitemexternalid', keep="last")
        ...
        try:
            ...
            Story=(Story.loc[Story['State'] == 'Closed'])
            Task=(Task.loc[Task['State'] == 'Closed'])
            storyeffort=set(Story['workitemexternalid']).intersection(set(Task['itemexternalid']))
            storyeffortdata=Task[Task['itemexternalid'].isin(list(storyeffort))]
            df4=storyeffortdata.groupby(['Iteration','WorkItemType'], as_index=False)['effortcompletedvalue','effortestimatedvalue','effortremainingvalue'].sum()
            df4=df4.drop(['WorkItemType'], axis = 1)
            df4.rename(columns={'effortestimatedvalue':'Story EffortEstimated','effortremainingvalue':'Story EffortRemaining','effortcompletedvalue':'Story EffortCompleted'}, inplace=True)
        except Exception as e:
            
            df4=pd.DataFrame()
            df4['Iteration'] = np.nan
            df4['Iteration'] = df4['Iteration'].astype('str')
        df5=(pd.merge(df3, df4, left_on='Iteration', right_on='Iteration', how='outer'))
        Story = Story.drop_duplicates(subset='UniqueRowID', keep="last")
        Story = Story.drop_duplicates(subset='prdctid_workitemexternalid', keep="last")
        Story['StoryPointEstimated'] = Story['StoryPointEstimated'].replace(np.nan,0)
        Story['StoryPointEstimated'] = Story['StoryPointEstimated'].astype(int)
    ...
    try: 
        ...
        data=Task.groupby(['Iteration','WorkItemType'], as_index=False)['EffortEstimated','EffortRemaining','EffortCompleted'].sum()
        data=data.drop(['WorkItemType'], axis = 1)
        data.rename(columns={'EffortEstimated':'Task EffortEstimated','EffortRemaining':'Task EffortRemaining','EffortCompleted':'Task CompletedEfforts'}, inplace=True)
        
        taskdf=(pd.merge(df11, data, left_on='Iteration', right_on='Iteration', how='outer'))
    ...
    try:
        ...
        if Defect['Iteration'].isnull().all():
            ...
            date['createdon'] = pd.to_datetime(date['createdon'], errors='coerce')
    ...
    try:
        ...
        #print("#####Agileaggdata######",Agileaggdata)
    try:
        if "Iterationname" not in Agileaggdata.columns:
            name=Agile[['Iteration','Iterationname']]
            name = name[name['Iteration'].notna()]
            name1=name.drop_duplicates(subset='Iteration', keep="last")
            Agileaggdata=(pd.merge(finaldf6,name1, left_on='Iteration', right_on='Iteration', how='left'))
    except Exception as e:
        error_encounterd = str(e.args[0])
...
def get_aggregated_data(cid,correlationId,dcuid,fromdate,todate,config,parent,Incremental,auto_retrain,flag= None):
    #auto_retrain = False
    ...
    for entity in  ['Story','Feature','Task','Issue','Epic','Impediment','Risk','Defect']:
        ...
        #agileAPI = agileAPI.format(urlencode(k))
        utils.CallCdmAPI(agileAPI,correlationId,entityDfs,entity,auto_retrain,lastDateDict,entityArgs,entityAccessToken)
        #print("call cdm")
    ...
    MultiSourceDfs,_ = utils.TransformEntities(MultiSourceDfs,cid,dcuid,EntityMappingColumns,parent,Incremental,auto_retrain,flag,correlationId = correlationId)
    ...
    utils.logger(logger, correlationId, 'INFO', ('AggregateAgileData '  + str(time.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
    iterationdf = entityDfs['Iteration']
    try:
        iterationdf['iterationuid'] = iterationdf['iterationuid'].astype('str')
        iterationdf['iterationuid'] = iterationdf['iterationuid'].str.replace('-','')
        iterationdf['iterationuid'] = iterationdf['iterationuid'].str.upper()
        iterationdf['iterationuid'] = iterationdf['iterationuid'].astype('str')
    except:
        error_encounterd = ''
    ...
    if isinstance(iterationdf, pd.DataFrame) and isinstance(Defectdatadf, pd.DataFrame):
        ...
        itertiondata = iterationdf[['iterationuid','starton','endon']]
        ...
        MultiSourceDfs['Entity']['Defect']['iterationuid'] = MultiSourceDfs['Entity']['Defect']['iterationuid'].astype('str')
        MultiSourceDfs['Entity']['Defect']['iterationuid'] = MultiSourceDfs['Entity']['Defect']['iterationuid'].str.replace('-','')
        MultiSourceDfs['Entity']['Defect']['iterationuid'] = MultiSourceDfs['Entity']['Defect']['iterationuid'].str.upper()
        itertiondata['iterationuid'] = itertiondata['iterationuid'].str.replace('-','')
        itertiondata['iterationuid'] = itertiondata['iterationuid'].str.upper()
        itertiondata['iterationuid'] = itertiondata['iterationuid'].astype('str')
        MultiSourceDfs['Entity']['Defect']['iterationuid'] = MultiSourceDfs['Entity']['Defect']['iterationuid'].astype('str')
        MultiSourceDfs['Entity']['Defect'] = (pd.merge(MultiSourceDfs['Entity']['Defect'], itertiondata, left_on='iterationuid', right_on='iterationuid', how='left'))
    ...
    if l!= []:
        ...
        Agile.rename(columns={'priorityuid':'Priority','iterationuid':'Iteration','iterationuidexternalvalue':'Iterationname','releaseuidexternalvalue':'Releasename','releaseuididvalue':'Release','effortcompleted':'EffortCompleted','effortestimated':'EffortEstimated','effortremaining':'EffortRemaining','businessvalue':'BusinessValue','storypointestimated':'StoryPointEstimated','starton':'StartOn','completedon':'CompletedOn','assignedatsourcetouser':'AssignedAtSourceToUser','teamarea':'TeamArea'}, inplace=True)
        #duplicates  = list(set(columns1) - set(columns2))
        Agile['Iteration'] = Agile['Iteration'].str.replace('-','')
        Agile['Iteration'] = Agile['Iteration'].str.upper()
        ...
        if isinstance(iterationdf, pd.DataFrame):
            iterationdf['iterationuid'] = iterationdf['iterationuid'].astype('str')
            itertiondata = iterationdf[['iterationuid','endon']]
            itertiondata.rename(columns={'endon':'iterationendon'}, inplace=True)
            ...
            itertiondata['iterationuid'] = itertiondata['iterationuid'].astype('str')
            Agile = (pd.merge(Agile,itertiondata, left_on='Iteration', right_on='iterationuid', how='left'))
            
        Agilesprintclosed = pd.DataFrame()
        Agilesprintactive = pd.DataFrame()
        Agilesprintclosed = Agile[Agile['Sprintstatus'].isin(['closed'])]
        Agilesprintclosed = Agilesprintclosed.drop_duplicates(subset='Iteration', keep="last")
        Agilesprintactive = Agile[Agile['Sprintstatus'].isin(['active'])]
        Agilesprintactive = Agilesprintactive.drop_duplicates(subset='Iteration', keep="last")
        ...
        #print("#####new##########",new)
        if Incremental:
            Agile=Agile[Agile['Iteration'].isin(list(Agilesprintactive['Iteration']))]
        if Incremental == False:
            Agile = Agile[Agile['Iteration'].isin(list(Agilesprintclosed['Iteration']))]
            
        if Agile.shape[0] != 0:
            aggData = Agileaggdata_function(Agile)
            lastDateDict['Entity'] = {}
            DateColumn = pd.to_datetime(Agiledata["DateColumn"].max()).strftime('%Y-%m-%d %H:%M:%S')
            lastDateDict['Entity']['DateColumn'] = DateColumn
            Agiledate = pd.DataFrame()
            Agiledata['DateColumn'] = Agiledata["DateColumn"].astype('datetime64[s]')
            Agiledate = Agiledata[["iterationuid","DateColumn"]]
            Agiledate["DateColumn"] = pd.to_datetime(Agiledate["DateColumn"], format="%Y-%m-%d %H:%M", exact=True)
            Agiledate_new = Agiledate[Agiledate.groupby("iterationuid").DateColumn.transform('max') == Agiledate['DateColumn']]
            Agiledate_new1 = Agiledate_new.sort_values('DateColumn').drop_duplicates('iterationuid',keep='last')
            Agiledate["iterationuid"] = Agiledate["iterationuid"].astype(str)
            Agiledate_new1['iterationuid'] = Agiledate_new1['iterationuid'].astype('str')
            Agiledate_new1['iterationuid'] = Agiledate_new1['iterationuid'].str.replace('-','')
            Agiledate_new1['iterationuid'] = Agiledate_new1['iterationuid'].str.upper()
            Agiledate_new1['iterationuid'] = Agiledate_new1['iterationuid'].astype('str')
            Agiledate_new1 = Agiledate_new1.drop_duplicates(subset='iterationuid', keep="last")
            Agiledate_new1['iterationuid'] = Agiledate_new1['iterationuid'].astype('str')
            aggData["Sprintid"] = aggData["Sprintid"].astype(str)
            aggData = (pd.merge(aggData,Agiledate_new1, left_on='Sprintid', right_on='iterationuid', how='left'))
            aggData=aggData.drop(['iterationuid'], axis = 1)
            aggData = aggData.drop(['Sprintid', 'Release'], axis=1, errors='ignore')
            listcol = ['Total Stories', 'story Completed', 'Story Remaining','Story EffortCompleted','Story EffortEstimated','Story EffortRemaining', 'Sprint Velocity', 'Planned Story Points','Feature Total', 'Feature Completed', 'Feature Remaining','Total Tasks', 'Task Completed', 'Task Remaining','Task EffortEstimated', 'Task EffortRemaining','Task CompletedEfforts','Issue Completed','Issue Total','Issue Remaining','Impediment Total', 'Impediment Completed','Impediment Remaining','Risk Total', 'Risk Completed', 'Risk Remaining','Epic Total', 'Epic Completed', 'Epic Remaining','Pre Sprint Defects','Post Sprint Defects','In Sprint Defects', 'Defect Total', 'Total Defects','Defect Remaining', 'Defect Estimated Efforts','Defect EffortRemaining', 'Defect EffortCompleted', 'average','New Defects', 'Active Defects','Total BusinessValue','Count of Critical and High items','Total closed critical','Team Size','Unique Team','Total StoryPointCompleted']
            aggData = aggData.reindex(aggData.columns.union(listcol, sort=False), axis=1, fill_value=np.nan)
            try:
                aggData = aggData[aggData['Sprint Name'].notna()]
                for col in aggData.columns:
                    if col not in ['DateColumn', 'First Defect Reported Date', 'Sprint Start Date', 'Sprint End Date']:
                        aggData[col].fillna(0, inplace=True)
            except Exception as e:
                error_encounterd = str(e.args[0])
