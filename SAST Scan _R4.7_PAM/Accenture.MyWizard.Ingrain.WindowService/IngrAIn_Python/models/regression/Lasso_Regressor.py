...
def lassoRegressor_RS_params():
    params={'alpha':[0.01, 0.05,0.1, 0.5, 1,2],
            'max_iter' : [3, 4, 5, 6, 7, 8, 9, 10],
			'tol'    : [i/10.0 for i in range(0,1)]}
    return params
...
def main(correlationId,modelName,pageInfo,userId,HyperTune=None,HTId=None,version=None):  
    ...
    # Fetching model parameters
        if HyperTune == 'True':
            ...
            MLDL_Model,_,ProblemType,traincols = utils.get_pickle_file(correlationId,'Lasso Regressor','MLDL_Model')                        
    ...
    # Pulling data
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Data Fetched at: '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
...
#        UniqueIdentifier = utils.getUniqueIdentifier(correlationId)
        UniqueIdentifier,UIDList = utils.getUniqueIdentifier(correlationId)#visualization changes
...
#            trainData = trainData.drop(remove_uniqueid, axis=1)
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Test Train split started at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Model Defination Completed, Training the model started at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        ...
        if HyperTune == 'True':            
         ...
         # Random search Cross Validate Model
            ...
            n_jobs= 1
            params = lassoRegressor_RS_params()
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Model training completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))        
        #print (model)
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor: Trained model saved at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))        
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Predictions done at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
    ...
    # changes for visualization
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Model Evaluation completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor :Model Feature Importance completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        ...
        utils.logger(logger,correlationId,'INFO',('Lasso Regressor : Model Engineering completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        utils.save_Py_Logs(logger,correlationId)       
