...
"Generalized Linear Model" : "False"
"Generalized Linear Model" : { families: [ "sm.families.Tweedie()",
...
import sys
sys.modules['scipy.misc.factorial'] = factorial
sys.modules['scipy.misc.logsumexp'] = logsumexp
sys.modules['scipy.misc.comb'] = comb
...
def model_train(X_train,Y_train,f):    ## Need changes   
    tweedie_model = sm.GLM(Y_train, X_train, family=eval(f))
    tweedie_model = tweedie_model.fit()
    return tweedie_model


def TrainModel(Y_train,X_train,Xtest,y_true):
    dtails = {}
    for f in fam:
        try:
            tweedie_model = sm.GLM(Y_train, X_train, family=eval(f))
            ...
            continue
        y_pred = tweedie_model.predict(Xtest)
        error = r2_score(y_true, y_pred)
        dtails[f] = error
    #print(dtails)
    model = max(dtails, key = dtails.get)
    #model = max(dtails)
    tweedie_model = sm.GLM(Y_train, X_train, family=eval(model))
    tweedie_model = tweedie_model.fit()
#        print("::::printing Model results:::::",error,i,f,tweedie_model.bic,tweedie_model.aic)
#        val = max(0,error)
#        if tweedie_model.bic < i :
#           # print(f,"less............")
#           # print(tweedie_model.bic)
#            dtails['r2'] = val
#            dtails['Model'] = tweedie_model
#            dtails['param'] = f
#            i =   tweedie_model.bic
#    print(dtails)
        
    return tweedie_model,model
...
def main(correlationId,modelName,pageInfo,userId,HyperTune=None,HTId=None,version=None):  
    ...
    # Fetching model parameters
        if HyperTune == 'True':
            ...
            data_jsonHT= list(dbcollection.find({"CorrelationId":correlationId,"HTId": HTId}))
            HTParams = data_jsonHT[0]['ModelParams']["families"]
            modelparams = HTParams                  
            MLDL_Model,_,ProblemType,traincols = utils.get_pickle_file(correlationId,'Generalized Linear Model','MLDL_Model')                        
    ...
    # Pulling data
        ...
        utils.logger(logger,correlationId,'INFO',('GLM : Data Fetched at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
...
#            trainData = trainData.drop(remove_uniqueid, axis=1)
        ...
        utils.logger(logger,correlationId,'INFO',('GLM : Test Train split started at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
        ...
        utils.logger(logger,correlationId,'INFO',('GLM : Model Defination Completed, Training the model at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        
        # Model training
        if HyperTune == 'True':  #### need chanes          
            model = model_train(X_train,Y_train,modelparams)  ## Need changes         
        else:               
            model ,prm = TrainModel(Y_train,X_train,X_test,Y_test)
#            model = details["Model"]
           
            modelparams =   {"families" :prm }
        utils.updQdb(correlationId,'P','60',pageInfo,userId,modelName = modelName,problemType=pType,UniId = HTId)
        utils.logger(logger,correlationId,'INFO',('GLM : Model training completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))        
        #print (model)
        ...
        utils.logger(logger,correlationId,'INFO',('GLM : Trained model saved at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))        
        ...
        utils.logger(logger,correlationId,'INFO',('GLM : Predictions done at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None))
        ...
        utils.logger(logger,correlationId,'INFO',('GLM :Model Evaluation completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
...
#                trainData.drop([UniqueIdentifier],axis = 1 ,inplace =True)
        ...
        utils.logger(logger,correlationId,'INFO',('GLM : Model Feature Importance processed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        ...
        utils.logger(logger,correlationId,'INFO',('GLM: Model Engineering completed at : '+ str(datetime.today().strftime('%Y-%m-%d %H:%M:%S'))),str(None)) 
        utils.save_Py_Logs(logger,correlationId)       
