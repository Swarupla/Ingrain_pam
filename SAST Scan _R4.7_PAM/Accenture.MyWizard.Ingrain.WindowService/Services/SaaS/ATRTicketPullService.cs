...
namespace Accenture.MyWizard.Ingrain.WindowService.Services
{
    ...
    #endregion
    public class ATRTicketPullService : ISaaSTicketPullService
    {
        private readonly WINSERVICEMODELS.AppSettings appSettings = null;
        private readonly IMongoDatabase _database;

        public ATRTicketPullService()
        {
        ...
        public List<ClientInfo> FetchProvisionedE2EID()
        {
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(ATRTicketPullService), "FetchProvisionedE2EID", ex.Message, ex.InnerException, string.Empty, string.Empty, string.Empty, string.Empty);
            }
        ...
        public ClientInfo FetchSaaSProvisionedDetails(ATRProvisionRequestDto atrdata)
        {
            ...
            {                
                LOGGING.LogManager.Logger.LogProcessInfo(typeof(ATRTicketPullService), "FetchSaaSProvisionedDetails", string.Format(CONSTANTS.MSG_NoProvisionForSaaS, atrdata.E2EUId), string.Empty, string.Empty, string.Empty, string.Empty);
            }
        ...
        /// <summary>
        /// Check in DB_PAMHistoricalAndDeltaPullTracker for In Progress/Completed records.
        /// </summary>
        /// <param name="ClientUId">The ClientUId</param>
        /// <param name="EndToEndUId">The EndToEndUId</param>
        /// <param name="entityType">The entityType</param>
        /// <param name="pullType">The pullType</param>
        /// <returns>Returns true if it has records else false</returns>
        public bool CheckPAMHistoricalPullTracker(string ClientUId, string EndToEndUId, string entityType, string pullType)
        {
            try
            {
                LOGGING.LogManager.Logger.LogProcessInfo(typeof(ATRTicketPullService), "CheckPAMHistoricalPullTracker", "Inside CheckPAMHistoricalPullTracker method", string.Empty, string.Empty, string.Empty, string.Empty);
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(ATRTicketPullService), "CheckPAMHistoricalPullTracker", Ex.Message, Ex.InnerException, string.Empty, string.Empty, string.Empty, string.Empty);
            }

            return false;
        }

        public bool DeleteHistory(string clientUId, string e2eUId, string entityType)//TODO:make void
        {
            bool status = false;
            try
            {
        ...
        public void InsertPAMHistoricalPullTracker(string clientUId, string EndToEndUId, string entityType, string pullType, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                var trackerFilter = collection.Find(c => c.ClientUId == clientUId && c.EndToEndUId == EndToEndUId && c.EntityType == entityType && c.PullType == pullType).ToList();
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(ATRTicketPullService), "InsertPAMHistoricalPullTracker", ex.Message, ex.InnerException, string.Empty, string.Empty, string.Empty, string.Empty);
                throw ex;
            }
        }
        ...
        public List<PAMHistoricalPullTracker> FetchPAMHistoricalPullTracker(string clientUId, string EndToEndUId, string entityType, string pullType)
        {
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(ATRTicketPullService), "fetchPAMHistoricalPullTracker", Ex.Message, Ex.InnerException, string.Empty, string.Empty, string.Empty, string.Empty);
                throw Ex;
            }
        }
        ...
        public void UpdatePAMHistoricalPullTracker(string ClientUId, string EndToEndUId, string entityType, string pullType, int flag, string statustext, int recordCount, DateTime? maxLastDFUpdatedDate)
        {
            try
            {
                LOGGING.LogManager.Logger.LogProcessInfo(typeof(ATRTicketPullService), nameof(UpdatePAMHistoricalPullTracker), "START", Guid.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                var collection = _database.GetCollection<PAMHistoricalPullTracker>(CONSTANTS.DB_PAMHistoricalAndDeltaPullTracker);
                var filterBuilder = Builders<PAMHistoricalPullTracker>.Filter;
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(ATRTicketPullService), nameof(UpdatePAMHistoricalPullTracker), ex.Message, ex, string.Empty, string.Empty, string.Empty, string.Empty);
                throw ex;
            }
        }
        ...
        public void InsertPAMHistoricalPullFailedTracker(PAMHistoricalPullTracker tracker)
        {
            try
            {
                LOGGING.LogManager.Logger.LogProcessInfo(typeof(ATRTicketPullService), nameof(InsertPAMHistoricalPullFailedTracker), "START", Guid.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(ATRTicketPullService), nameof(InsertPAMHistoricalPullFailedTracker), ex.Message, ex, string.Empty, string.Empty, string.Empty, string.Empty);
                throw ex;
            }
        }
