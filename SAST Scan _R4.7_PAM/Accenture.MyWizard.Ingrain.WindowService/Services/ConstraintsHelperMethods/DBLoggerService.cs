...
namespace Accenture.MyWizard.Ingrain.WindowService.Services.ConstraintsHelperMethods
{
    class DBLoggerService
    {
        ...
        public string UpdateOfflineException(DATAMODELS.DeployModelsDto item, string errorMessage)
        {
            ...
            if (result.Count > 0)
            {
                var update = Builders<DATAMODELS.DeployModelsDto>.Update.Set("offlineTrainingRunDate", DateTime.UtcNow.ToString()).Set("ExceptionDate", DateTime.UtcNow.ToString()).Set("ConstraintsExceptionMessage", errorMessage);
                collection.UpdateOne(filter, update);
            }

            return "Success";
        }
         public string UpdateOfflineException(DATAMODELS.GenericTrainingRequestDetails item, string errorMessage)
        {
            var collection = _database.GetCollection<DATAMODELS.DeployModelsDto>(CONSTANTS.SSAIDeployedModels);
            var filter = Builders<DATAMODELS.DeployModelsDto>.Filter.Eq("CorrelationId", item.CorrelationId);
            var result = collection.Find(filter).ToList();
        ...
        public string UpdateAIOfflineException(AIDataModels.UseCase.UsecaseDetails item, string errorMessage)
        {
            ...
            if (result.Count > 0)
            {
                var update = Builders<AIDataModels.UseCase.UsecaseDetails>.Update.Set("offlineTrainingRunDate", DateTime.UtcNow.ToString()).Set("ExceptionDate", DateTime.UtcNow.ToString()).Set("ConstraintsExceptionMessage", errorMessage);
                collection.UpdateOne(filter, update);
            }
        ...
        public string UpdateOfflinePredException(DATAMODELS.DeployModelsDto item, string errorMessage)
        {
            ...
            if (result.Count > 0)
            {
                var update = Builders<DATAMODELS.DeployModelsDto>.Update.Set("offlinePredRunDate", DateTime.UtcNow.ToString()).Set("ExceptionDate", DateTime.UtcNow.ToString()).Set("ConstraintsExceptionMessage", errorMessage);
                collection.UpdateOne(filter, update);
            }
