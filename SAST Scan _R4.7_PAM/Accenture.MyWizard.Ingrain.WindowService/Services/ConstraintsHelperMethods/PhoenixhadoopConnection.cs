...
namespace Accenture.MyWizard.Ingrain.WindowService.Services.ConstraintsHelperMethods
{
    class PhoenixhadoopConnection
    {
        ...
        private readonly string _hadoopCRApi = "v1/ChangeRequests?clientUId={0}&deliveryConstructUId={1}&CorrelationUId={2}&CorrelationBatchId={3}";
        ...
        public WINSERVICEMODELS.ConstructsDTO FetchTeamAreaUID(string ClientUID, string DeliveryConstructUID, string AppserviceUID)
        {
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(PhoenixhadoopConnection), nameof(FetchTeamAreaUID), ex.Message, ex, "", string.Empty, ClientUID, DeliveryConstructUID, CONSTANTS.CustomConstraintsLog);
            }
            return TeamAreaUIdAtDCLevel;
        }
        ...
        {
            WINSERVICEMODELS.IterationsArrayDTO oIterations = null;
            ...
            try
            {
                ...
                if (result != null)
                {
                    ...
                    oIterations = JsonConvert.DeserializeObject<WINSERVICEMODELS.IterationsArrayDTO>(response.ToString());
                    ...
                    foreach (WINSERVICEMODELS.IterationsDTO oIteration in oIterations.Iterations)
                    {
                        if (ServiceType == CONSTANTS.TrainingConstraint)
                        {
                            if (Convert.ToDateTime(oIteration.EndOn) < DateTime.Now)
                            {
                                ...
                                if (!string.IsNullOrEmpty(oIteration.IterationUId) && !sIterationUID.Contains(oIteration.IterationUId))
                                {
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(PhoenixhadoopConnection), nameof(FetchClosedSprintandIterations), ex.Message, ex, string.Empty, string.Empty, "ClientId : " + ClientUID, "DeliveryConstructUID : " + DeliveryConstructUID, CONSTANTS.CustomConstraintsLog);
            }
            finally
            {
                sIterationUID = null;
        ...
        public void FetchPhoenixIterationsList(dynamic item, string ServiceLevel)
        {
            DATAMODELS.DeployModelsDto oDeployedModel = null;
            AIDataModels.UseCase.UsecaseDetails oUseCaseDetails = null;

            try
            {
                var uniqueClientsDCs1 = new List<(string CorrelationId, string TemplateUsecaseId, string ClientUId, string DeliveryConstructUID)>();

                // fetch the list of Trained models 
                if (ServiceLevel.ToUpper() == CONSTANTS.SSAIAPP.ToUpper())
                {
                    oDeployedModel = item;
                    var filter = Builders<DATAMODELS.DeployModelsDto>.Filter.Eq("TemplateUsecaseId", oDeployedModel.CorrelationId.ToString())
                               & Builders<DATAMODELS.DeployModelsDto>.Filter.Eq("Status", CONSTANTS.Deployed);
                    List<DATAMODELS.DeployModelsDto> deployModels = _DBConnection.GetCollectionData<DATAMODELS.DeployModelsDto>(filter, CONSTANTS.SSAIDeployedModels);
                        ...
                        if (uniqueClientsDCs.Count > 0)
                        {
                            foreach (var dc in uniqueClientsDCs)
                            {
                                ...
                                if (!string.IsNullOrEmpty(result))
                                {
                                    ...
                                    _DBConnection.UpdateIterationstoDB(iterationQueryResponse, dc.ClientUId, dc.DeliveryConstructUID, dc.CorrelationId, dc.TemplateUsecaseId);
                                    ...
                                    if (iterationQueryResponse.TotalPageCount > 1)
                                    {
                                        for (int i = 2; i < iterationQueryResponse.TotalPageCount + 1; i++)
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(result1))
                                            {
                                                ...
                                                _DBConnection.UpdateIterationstoDB(iterationQueryResponse1, dc.ClientUId, dc.DeliveryConstructUID, dc.CorrelationId, dc.TemplateUsecaseId);
                                            }
                                            else
                                            {
                                                break;
                ...
                {
                    oUseCaseDetails = item;
                    // fetch the list of Trained models
                    var filter = Builders<AIDataModels.AICoreModels>.Filter.Eq("TemplateUsecaseId", oUseCaseDetails.CorrelationId.ToString())
                                 & Builders<AIDataModels.AICoreModels>.Filter.Eq("Status", CONSTANTS.Deployed);
                    List<AIDataModels.AICoreModels> deployModels = _DBConnection.GetCollectionData<AIDataModels.AICoreModels>(filter, CONSTANTS.AICoreModels);
                    ...
                    if (deployModels.Count > 0)
                    {
                        ...
                        if (uniqueClientsDCs.Count > 0)
                        {
                            foreach (var dc in uniqueClientsDCs)
                            {
                                ...
                                result = InvokeIterationsQueryAPI(dc.ClientId, dc.DeliveryConstructId, 1); // endon shld be futuredata

                                if (!string.IsNullOrEmpty(result))
                                {
                                    iterationQueryResponse = JsonConvert.DeserializeObject<WINSERVICEMODELS.PhoenixPayloads.IterationQueryResponse>(result);
                                    _DBConnection.UpdateIterationstoDB(iterationQueryResponse, dc.ClientId, dc.DeliveryConstructId, dc.CorrelationId, dc.UsecaseId);

                                    if (iterationQueryResponse.TotalPageCount > 1)
                                    {
                                        for (int i = 2; i < iterationQueryResponse.TotalPageCount + 1; i++)
                                        {
                                            string result1 = InvokeIterationsQueryAPI(dc.ClientId, dc.DeliveryConstructId, i);
                                            if (!string.IsNullOrEmpty(result1))
                                            {
                                                WINSERVICEMODELS.PhoenixPayloads.IterationQueryResponse iterationQueryResponse1
                                                = JsonConvert.DeserializeObject<WINSERVICEMODELS.PhoenixPayloads.IterationQueryResponse>(result1);
                                                _DBConnection.UpdateIterationstoDB(iterationQueryResponse1, dc.ClientId, dc.DeliveryConstructId, dc.CorrelationId, dc.UsecaseId);
                                            }
                                            else
                                            {
                                                break;
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(PhoenixhadoopConnection), nameof(FetchPhoenixIterationsList), ex.Message, ex, string.Empty, string.Empty, string.Empty, string.Empty, CONSTANTS.CustomConstraintsLog);
            }
            finally
            {
                oDeployedModel = null;
                oUseCaseDetails = null;
            }
        }
        ...
        public Dictionary<string, List<string>> FetchClosedUserStory(string ClientUID, string DeliveryConstructUID, string ProvisionedAppServiceUID)
        {
                    ...
                    if (!string.IsNullOrEmpty(jsonResult))
                    {
                        ...
                        if (ClosedUserStoryQueryResponse.Entity != null)
                        {
                            ...
                            foreach (WINSERVICEMODELS.EntityDTO oEntity in ClosedUserStoryQueryResponse.Entity)
                            {
                                ...
                                {
                                    if (!string.IsNullOrEmpty(oEntity.Iterationuid) && !sClosedUserStoryUID.Contains(oEntity.Iterationuid))
                                    {
            ...
            {
                LOGGING.LogManager.Logger.LogErrorMessage(typeof(PhoenixhadoopConnection), nameof(FetchClosedUserStory), ex.Message, ex, string.Empty, string.Empty, "ClientId : " + ClientUID, "DeliveryConstructUID : " + DeliveryConstructUID, CONSTANTS.CustomConstraintsLog);
            }
        ...
        {
            LOGGING.LogManager.Logger.LogProcessInfo(typeof(PhoenixhadoopConnection), nameof(GetValidClientId), "FETCH CLIENTS START", "", "", "", "", CONSTANTS.CustomConstraintsLog);
            string clientUId = null;
            string routeUrl = "AccountClients?clientUId=" + appSettings.ClientUID + "&deliveryConstructUId=null";

            var jsonStringResult = _HttpMethodService.InvokeGetMethod(routeUrl, appserviceUId);
        ...
        public string GetEntityPayload(string clientUId, string deliveryConstructUId, string entityName, string itemUId)
        {
            string token = _TokenService.GenerateToken();
            string jsonResult = string.Empty;
        ...
        public string UpdateEntityRecommendationInPhoenix(dynamic payload, string clientUId, string deliveryConstructUId, string correlationUId, string entityName)
        {
            string jsonResult = string.Empty;
            ...
            HttpResponseMessage response = _HttpMethodService.InvokePOSTRequest(token, appSettings.myWizardAPIUrl, apiPath, content, correlationUId);
